...
.then(data => {
    // Imposta il titolo e il favicon
    document.title = data.title;
    document.getElementById("favicon").href = data.meta.icon;

    // Imposta il testo di benvenuto
    document.getElementById("welcome-text").innerText = `BENVENUTI NELLA ${data.title} DI DAVIDE SOAVE!`;

    // Costruisci la navbar
    const navbarLinks = document.getElementById("navbar-links");
    let navContent = `<a class="navbar-brand" href="../index/index.html">${data.nav.brand}</a><div class="d-flex">`;
    ...
    // Costruisci le card principali
    const mainContent = document.getElementById("main-content");
    data.mainContent.cards.forEach(card => {
        mainContent.innerHTML += `
            <div class="col-md-6 col-lg-4 mb-4">
                <div class="card">
                    <img style="height: 14rem; object-fit: cover;" src="${card.image}" class="card-img-top" alt="${card.title}">
                    <div class="card-body">
